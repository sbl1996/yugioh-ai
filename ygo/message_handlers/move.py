...
def move(duel: Duel, code, location, newloc, reason):
	...
	if reason & REASON.DESTROY and card.location != cnew.location:
		duel.inform(
			pl,
			(INFORM.PLAYER, lambda p: p._("Card %s (%s) destroyed.") % (plspec, card.get_name(p))),
			(INFORM.OPPONENT, lambda p: p._("Card %s (%s) destroyed.") % (opspec, card.get_name(p)))
		)
		...
		if card.controller != cnew.controller:
			...
			duel.inform(
				...
				(INFORM.OPPONENT, lambda p: p._("{plname}s card {spec} ({name}) changed its zone to {targetspec}.").format(plname=pl.nickname, spec=getspec(p), targetspec=getnewspec(p), name=card.get_name(p))),
			)
		...
		duel.inform(
			...
			(INFORM.OPPONENT, lambda p: p._("{plname} discarded {spec} ({name}).").format(plname=pl.nickname, spec=getspec(p), name=card.get_name(p))),
		)
		...
		duel.inform(
			...
			(INFORM.OPPONENT, lambda p: p._("{plname}'s banished card {spec} ({name}) returned to their field at {targetspec}.").format(plname=pl.nickname, spec=getspec(p), targetspec=getnewspec(p), name=card.get_name(p))),
		)
		...
		duel.inform(
			...
			(INFORM.OPPONENT, lambda p: p._("{plname}s card {spec} ({name}) returns from the graveyard to the field at {targetspec}.").format(plname = pl.nickname, spec=getspec(p), targetspec=getnewspec(p), name = card.get_name(p))),
		)
		...
		duel.inform(
			...
			(INFORM.OPPONENT, lambda p: p._("%s detached %s")%(pl.nickname, card.get_name(p))),
		)
		...
		duel.inform(
			...
			(INFORM.OPPONENT, lambda p: p._("{plname}'s card {spec} ({name}) was sent to the graveyard.").format(plname=pl.nickname, spec=getspec(p), name=card.get_name(p))),
		)
		...
		duel.inform(
			...
			(INFORM.OPPONENT, lambda p: p._("{plname}'s card {spec} ({name}) returned to their extra deck.").format(plname=pl.nickname, spec=getspec(p), name=card.get_name(p))),
		)
		...
		duel.inform(
			...
			(INFORM.OPPONENT, lambda p: p._("{plname}'s card {spec} ({name}) was attached to {targetspec} ({targetname}) as XYZ material").format(spec=getspec(p), name=card.get_name(p), targetspec=attached_to.get_spec(p), targetname=attached_to.get_name(p), plname=pl.nickname)),
		)
